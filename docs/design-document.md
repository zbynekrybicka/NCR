...
**Nádrže a čerpadla** — Tvar a kapacita nádrže se odvozuje ze sestavy zdí, které ji ohraničují (nádrž nelze umístit volně): autor levelu označí kteroukoli kostku uzavřené dutiny a tím z ní udělá nádrž. Dutina, ze které by voda vytekla (dotýká se okraje levelu nebo neuzavřené stěny), nádrží být nemůže. Kromě tvaru se u nádrže nastavuje **počáteční objem vody** (v půlkostkových jednotkách, aby šla zapsat i hladina uprostřed kostky a půlkostkový objem šikminy; 1 kostka = 2 jednotky) — počáteční objem nesmí přesáhnout kapacitu nádrže. Lze nastavit kapacitu jako neomezenou (hladina se pak nikdy nemění a čerpadlo z ní čerpat nesmí).

Čerpadlo se definuje označením dvou nádrží (vzdálenost neomezená, model nepovinný), napojuje se na **jednu i více elektrických skříní** a případně na řídicí jednotku; definuje se defaultní zdrojová/cílová nádrž, resp. defaultní směr u obousměrného čerpadla. Jedno sepnutí přečerpá jednu kostku vody. Čerpadlo funguje jen tehdy, jsou-li **všechny** napojené skříně opravené a pod napětím.

Čerpadlo napojené jen na skříň (bez řídicí jednotky) je **automatické**: přečerpá jednorázově v okamžiku, kdy jsou poprvé splněné všechny podmínky přenosu — všechny napojené skříně jsou opravené a pod napětím, ve zdrojové nádrži je aspoň kostka vody, v cílové aspoň kostka volné kapacity a přenos by žádného robota neutopil (viz [Maximální bezpečná hladina](#214-statické-prvky--překážky)). Jde tedy o sledování náběžné hrany: dokud podmínka platí dál, čerpadlo už znovu nesepne; sepne až poté, co podmínka mezitím přestala platit a je splněná znovu.

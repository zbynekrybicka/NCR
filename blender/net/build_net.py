...
animovat, a zkontroluje, že se model vejde do jedné buňky mřížky.
    ...
    NET_Root                        (Empty ve středu buňky)
     └─ NET_Body                    tělo nese úplně všechno
         ├─ CORE_net_*              jádro zapuštěné do hřbetu
         ├─ NET_LegFemur_L0..P2     kyčel: rotace kolem kloubu
         │   └─ NET_LegTibia_*      koleno: rotace kolem kloubu
         ├─ NET_CargoFloor
         │   └─ NET_Cargo_0..3      náklad, skrytý podle počtu
         └─ NET_Carapace            krunýř: rotace kolem X (čep vpředu)

Net nemá obličej, oči ani kusadla — viz zadání v net_spec.py. Orientaci
nese zúžená příď, sklon nohou a nápis na jádru.
"""
...
body = _ncr_import("part_01_body")
legs = _ncr_import("part_02_legs")
carapace = _ncr_import("part_03_carapace")
cargo = _ncr_import("part_04_cargo")
walk_anim = _ncr_import("anim_walk")


# --- co se má stát po sestavení --------------------------------------------
EXPORT_GLB = False       # True = zapsat game/assets/robots/net.glb
ANIMATE = True           # True = přidat klipy chůze a otáček (anim_walk.py)
APPLY_ROTATIONS = False  # True = zapéct rotace do meshů (až těsně před exportem)
CARGO_COUNT = None       # None = hodnota z net_spec; jinak 0..4 předmětů
...
def build():
    ...
    net_coll = nc.collection("NET")
    carried = S.CARGO_COUNT if CARGO_COUNT is None else CARGO_COUNT

    parts = []
    parts += body.build(net_coll)
    parts += legs.build(net_coll)
    parts += cargo.build(net_coll, count=carried)
    # krunýř se staví dosednutý a otevře se až po zavěšení, jinak by se
    # otočil rodič a náklad zůstal stát
    parts += carapace.build(net_coll, open_angle=0.0)
    ...
    nc.place(root, net_coll)

    # --- hierarchie --------------------------------------------------------
    femurs = [o.name for o in parts if o.name.startswith("NET_LegFemur_")]
    _parent_many("NET_Root", ["NET_Body"])
    _parent_many("NET_Body", ["NET_Carapace", "NET_CargoFloor"] + femurs
                 + [o.name for o in core_parts])
    for name in femurs:
        _parent_many(name, [name.replace("Femur", "Tibia")])
    _parent_many("NET_CargoFloor",
                 [o.name for o in parts if o.name.startswith("NET_Cargo_")])

    # spec §4: 0-2 předměty se pod krunýř vejdou, 3-4 už ne
    angle = S.CARGO_OPEN_ANGLE if carried >= S.CARGO_OPEN_FROM else S.CARAPACE_OPEN
    if angle:
        shell = obj("NET_Carapace")
...
def report(parts):
    ...
    print("[NCR] přísavky dosedají na Z = %.3f (má být -0.500), příď k -Y" % lo[2])
...
def export_glb(path=None):
    ...
    if path is None:
        ...
        path = os.path.join(repo, "game", "assets", "robots", "net.glb")
    ...
    targets = [o for o in bpy.data.objects
               if o.name.startswith(("NET_", "CORE_%s_" % S.ROBOT))]
    nc.prepare()

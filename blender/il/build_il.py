# -*- coding: utf-8 -*-
"""
Il — kompletní sestavení.

    IL_Root                         (Empty ve středu buňky)
     ├─ IL_Leg_L / _P / _C          nohy
     │   └─ IL_Wheel_L / _P / _C    jízda: rotace kolem X
     └─ IL_Body
         ├─ CORE_il_*               jádro = hlava (spec §7)
         ├─ IL_ArmHousing
         ├─ IL_ArmSolder            pájecí rameno, výsuv podél osy
         └─ IL_ArmUSB               USB rameno, výsuv podél osy

Il je jediný robot, kde hlava a jádro splývají — koule sedí přímo na
trupu, žádná kupole navíc.
"""
...
arms = _ncr_import("part_03_arms")


# --- co se má stát po sestavení --------------------------------------------
EXPORT_GLB = False       # True = zapsat game/assets/robots/il.glb
APPLY_ROTATIONS = False  # True = zapéct rotace do meshů (až těsně před exportem)
EXTEND = None            # None = hodnota z il_spec; 0 = ramena zasunutá
...
def build():
    ...
    il_coll = nc.collection("IL")

    parts = []
    parts += body.build(il_coll)
    parts += legs.build(il_coll)
    parts += arms.build(il_coll, extend=EXTEND)
    ...
    nc.purge(("IL_Root",))
    root = bpy.data.objects.new("IL_Root", None)
    root.empty_display_type = 'PLAIN_AXES'
    root.empty_display_size = 0.5
    nc.place(root, il_coll)

    _parent_many("IL_Root", ["IL_Body", "IL_Leg_L", "IL_Leg_P", "IL_Leg_C"])
    _parent_many("IL_Body", ["IL_ArmHousing", "IL_ArmSolder", "IL_ArmUSB"]
                 + [o.name for o in core_parts])
    for tag in ("L", "P", "C"):
        _parent_many("IL_Leg_" + tag, ["IL_Wheel_" + tag])
...
def export_glb(path=None):
    ...
    if path is None:
        ...
        path = os.path.join(repo, "game", "assets", "robots", "il.glb")
    ...
    targets = [o for o in bpy.data.objects
               if o.name.startswith(("IL_", "CORE_%s_" % S.ROBOT))]
    nc.prepare()
